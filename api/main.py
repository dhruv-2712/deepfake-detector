...
async def load_models():
    global _img_ext, _aud_ext, _vid_ext, _fusion, _device
    ...
    if ckpt.exists():
        ...
        _fusion.load_state_dict(state.get("fusion", {}),         strict=False)
...
def _score(img_emb=None, aud_emb=None, vid_emb=None) -> dict:
    with torch.no_grad():
        prob = _fusion(img_emb=img_emb, aud_emb=aud_emb, vid_emb=vid_emb).squeeze().item()
